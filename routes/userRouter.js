...
// router.post("/:id/cart", [auth, getBooking], async (req, res, next) => {
...
//   const user = await User.findById(req.user._id);
//   // console.log(user)
//   let booking_id = res.booking._id;
//   let title = res.booking.title;
//   let category = res.booking.category;
//   let img = res.booking.img;
//   let price = res.booking.price;
...
router.put("/cart", [auth, getBooking], async (req, res, next) => {
  const user = await User.findById(req.user._id);
